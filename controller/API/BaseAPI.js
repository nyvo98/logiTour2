...
export default class BaseAPI {
  ...
  static async getConfigByTypeMultiple (body) {
    return this.postGateWay('config/typeMulti', REQUEST_TYPE.POST, { setting: body })
  }

  static async postGateWay (action, method = REQUEST_TYPE.GET, body, queryBody, tokenUser) {
    try {
      let serverUrl = process.env.REACT_APP_API
      ...
      console.log(error)
      if (tokenUser) {
        return error.message === 'Request failed with status code 401' ? 'authenFailed' : null
      } else {
        return null
      }
    }
