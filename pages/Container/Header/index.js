...
class Header extends React.PureComponent {
  ...
  menu = () => {
    ...
    let selectOptionList = arrLanguage.filter(
      obj => obj.lang !== lang
    )
    return (
      <React.Fragment>
  ...
  renderJumpBotron = () => {
    ...
    return (
      <div className={`header-container__header`} style={{ backgroundImage: `linear-gradient(to bottom, #020815 0%, rgba(2, 8, 21, 0) 100%), url(${headerObject.image || globalHeader.background})`, minHeight: `${globalHeader.height || 70}vh` }}>
        <div className='header-container__textbox'>
          <div className='heading-primary'>
            <div className='container'>
              <h1 className='heading-primary--main' dangerouslySetInnerHTML={{ __html: getNameObject(headerObject.title, locale.lang) || globalHeader.mainTitle }} />
              <h2 className='heading-primary--sub' dangerouslySetInnerHTML={{ __html: getNameObject(headerObject.subTitle, locale.lang) || globalHeader.subTitle }} />
            </div>
          </div>
        </div>
      </div>
  ...
  onShowLoginPopup = () => {
    this.myModal.current.openModal(<LoginPopup closeModal={this.myModal.current.closeModal} />, 563)
  }
  ...
  onShowContactPopup=() => {
    this.myModal.current.openModal(<ContactusPopup closeModal={this.myModal.current.closeModal} />, 900)
  }
  ...
  renderDeskopNavBar () {
    ...
    return (
      <>
        <div className={`header-navigation ${isScroll ? 'header-navigation--fixed' : ''}`}>
          <div className={`header-container__topbar`}>
            <div className='container flex'>
              <Row style={{ width: '100%' }} >
                ...
                17-19 Munjeongno 5Gil, Songpa-gu, Seoul
                </Col>
                <Col flex={1} className='flex align-center'>
                  <i className='icon icon--phone-fullfill MR10 icon--14 icon--primary' /> FAX : +82 02-409-9086
                </Col>
                <Col flex={1}>
                ...
                <Col flex={4} className='text text-right'>
                  <Menu mode='horizontal' theme='dark' className='header-background' style={{ lineHeight: '5rem', fontSize: '1.3rem' }}>
                    <Menu.Item>
                      <SelectLanguage fix />
                    </Menu.Item>
                    <Menu.Item>
                      {/* {userRedux ? (<Link href='/my-page'>{(messages.welcome) + ' ' + userRedux.email}</Link>) : (<a onClick={this.onShowLoginPopup}>{messages.login}</a>)} */}
                      {userRedux ? (<Link href='/my-page'>{messages.myAccount || 'My account'}</Link>) : (<a onClick={this.onShowLoginPopup}>{messages.login}</a>)}
                    </Menu.Item>
                  </Menu>
                </Col>
              </Row>
          ...
          <Media query='(min-width: 769px)' render={() => (
            <div className='container header-navigation__nav'>
              <Menu mode='horizontal' theme='dark'
                style={{ background: 'transparent', color: '#fff', border: 'none', fontSize: '1.3rem', lineHeight: isScroll ? null : '8.4rem' }} className='text text-right'>
                ...
                <Menu.Item>
                  <a onClick={this.onShowContactPopup} className='flex justify-center align-center primary' style={{ display: 'flex', color: '#d0af73' }}>
                    <i className='icon icon--envelop icon--34 icon--primary' />
                    {messages.contactUs}</a>
                </Menu.Item>
              </Menu>
            </div>
          )} />
