...
class AboutUs extends PureComponent {
    ...
    return contents.map((item) => {
      if (item.year) {
        ...
        return this.renderLine(
          item,
          mergeData.indexOf(item.id) % 2 === 0 ? 'left' : 'right'
        )
      }
    })
  }
  ...
  renderTimelineItemMobile = (contents) => {
    return contents.map((item) => {
      if (item.year) {
        ...
        return (
          <Timeline.Item
            dot={<div className="time-line-dot" />}
            color="#6d9cf4"
          >
            <p>
              ...
              <span className="aboutus-timeline__detail">
                {getNameObject(item.content, this.props.locale.lang)}
              </span>
            </p>
            <br />
          </Timeline.Item>
    ...
    return (
      ...
      >
        <div className={`aboutus-timeline__${cssPosition}-side`}>
          <div
            className={`aboutus-timeline__${cssPosition}-side__line-${cssPosition}`}
          >
            <img
  ...
  renderCompanyMember = () => {
    ...
    return (
      member &&
      member
        .sort((a, b) => a.numSort - b.numSort)
        .map(({ id, image, role, name }) => {
          return (
            <Col key={id} span={6} xs={24} lg={6} className="memberColumn">
              <div
    ...
    return (
      <div className="container">
        ...
        <p className="MT30 haveTrip">
          This is our test page. We will try to improve in the future. Thanks
                ...
                {settingRedux &&
                  settingRedux.timelineConfig.data.map((item) =>
                    this.renderTimelineItem(
                      item.info,
                      settingRedux.timelineConfig.mergeContent
                    )
                  )}
              </Timeline>
            </div>
