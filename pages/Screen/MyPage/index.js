...
class MyPage extends PureComponent {
  ...
  async componentDidMount () {
    ...
    try {
      ...
      const bookingList = response.slice(0, 4).map(booking => {
          ...
          <Row gutter={[30, 80]}>
            <Col xs={24} lg={12}>
              <Card className='card-custom' onClick={this.handleGoToInfo}>
                ...
                <h2 className='heading heading--main MB20'>{messages.myInformation || ''}</h2>
                <p className='gray MB50'>{messages.myInformationSub || ''}</p>
                <MyButton onClick={this.handleGoToInfo} isCard isFullWidth title={messages.infoMore || ''} className='card-custom__button' />
              </Card>
            </Col>
            <Col xs={24} lg={12}>
              <Card className='card-custom card-custom--scale' onClick={this.handleGoToAccount}>
                <div className='card-custom__ic MB20 MT20'>
                  <i className='icon icon--setting-account icon--34' />
                </div>
                <h2 className='card-custom__title heading heading--main MB20'>{messages.myAccount || ''}</h2>
                <p className='gray MB50'>{messages.myAccountSub || ''}</p>
                <MyButton onClick={this.handleGoToAccount} isFullWidth title={messages.accountMore || ''} className='card-custom__button' />
              </Card>
            </Col>
          </Row>
          <h2 className='heading heading--main PB50'>{messages.bookingHistory || ''}</h2>
        ...
        <Row gutter={[50, 50]}>
          <Col xs={24} lg={7}>
            <Card className='card-custom'>
              <img src={images.icProfileDefault} alt='' className='card-custom__avatar' />
              <h2 className='heading heading--main MB5'>{userRedux ? userRedux.firstName + ' ' + userRedux.lastName : ''}</h2>
              ...
              <div className='my-page-container__button'>
                <MyButton isFullWidth title={messages.logout || ''} onClick={this.onBtnLogoutClick} />
              </div>
            </Card>
          </Col>
          <Col xs={24} lg={17}>
