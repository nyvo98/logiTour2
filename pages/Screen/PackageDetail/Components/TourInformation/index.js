...
class TourInformation extends React.PureComponent {
  ...
  renderLightBox = () => {
    ...
    if (image[currentImage].type === packageImageType.video) {
      video =
        image[currentImage].url && image[currentImage].url.split('?v=')[1]
    }
    return (
        ...
        <div className='lightbox__container' onClick={this.onLightBoxClick}>
          ...
          {image[currentImage].type === packageImageType.image ? (
            ...
            <div
              ...
              className='lightbox__videoPlayer'
            >
              <iframe
                width='100%'
                height='100%'
    ...
    return (
      <div className='tour-information-container'>
          ...
          <Row gutter={[50, 50]}>
            <Col xs={24} lg={12} xl={6}>
              <div className='tour-detail__box'>
                ...
                <h4>
                  {moment(data.duration.from).format('DD/MM/YYYY').toUpperCase()} {' - '} {moment(data.duration.to).format('DD/MM/YYYY').toUpperCase()} {' '}
                </h4>
              </div>
            </Col>
            <Col xs={24} lg={12} xl={6}>
              <div className='tour-detail__box'>
                ...
                <h4>{`${data.numPerson.from || '10'}~${
                  data.numPerson.to || '100'
                }`}</h4>
              </div>
            </Col>
            <Col xs={24} lg={12} xl={6}>
            ...
            <Col xs={24} lg={12} xl={6}>
              <div className='tour-detail__box'>
                ...
                <h6>{messages.from || ''}</h6>
                <h4 className='fz--18'>
                  VND{formatNumberBro(price)} {messages.perPerson || ''}
                </h4>
              </div>
            </Col>
          </Row>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: getNameObject(description, lang) }}
        />
      </div>
    )
  }
