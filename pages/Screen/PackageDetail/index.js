...
import { getNameObject, formatNumberBro, calculateDiffDate, getLength, stripHtml } from 'common/function'
...
class PackageDetail extends React.PureComponent {
  ...
  async componentDidMount () {
    ...
    const tourDetail = await BaseAPI.getDataByMe('tour', this.props.router.query.id)
    if (tourDetail) {
      const best = calculateDiffDate(tourDetail.tourInfoList.bestDuration.to, tourDetail.tourInfoList.bestDuration.from, 'days')
      const normal = calculateDiffDate(tourDetail.tourInfoList.duration.to, tourDetail.tourInfoList.duration.from, 'days')
    ...
    const paymentInformation = await BaseAPI.getData(`bookingHistory/tourPayment/${this.props.router.query.id}`)
  ...
  handleBooking = (tour) => () => {
    ...
    return this.ref.current.openModal(<BookingModal tour={tour} handlePayment={this.handlePaypalBooking} />, 900)
  }

  handlePaypalBooking = (tour) => () => {
    const { paymentInformation } = this.state
    this.setState({ paymentInformation: { ...paymentInformation, ...tour } }, () => {
      return this.handleOpenPaypal(tour)()
    })
  }
  ...
  handleOpenPaypal = (tour) => () => {
    ...
    return this.ref.current.openModal(<PaypalBookingModal isPayment={paymentInformation ? paymentInformation.isPayment : null} tour={payload} />, 900)
  }

  handlePaypalSuccess = (isFail) => {
    return this.ref.current.openModal(<PaypalBookingModal isSuccess={isFail} />, 900)
  }

  render () {
    const { isTooltipVisible, tour, duration } = this.state
    const { _id, title, image, description, contactList, subDescription, price, tourInfoList, tourScheduleList, bookingInfoList } = tour
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(PackageDetail))
